//
//  DogPersonalProfileViewController.swift
//  PenPaws
//
...
class DogPersonalProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        _ = databaseRef.child("dogs").child(dogIdentifier).observe(.value, with: { (snapshot) in
            ...
            self.dogOwnerName.text = value?["owner"] as? String ?? ""
    ...
    public override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "toDonation"){
            let secondScene = segue.destination as! PopUpViewController
